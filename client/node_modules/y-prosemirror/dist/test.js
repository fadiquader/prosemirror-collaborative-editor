(function () {
  ...
  */
  ...
  const addToDeleteSet = (ds, id, length) => {
    setIfUndefined$1(ds.clients, id.client, () => []).push(new DeleteItem(id.clock, length));
  };
   ...
   * @return {boolean}
   */
  const equalFlat = (a, b) => a === b || (length$2(a) === length$2(b) && every(a, (val, key) => (val !== undefined || hasProperty$1(b, key)) && b[key] === val));
  ...
  class ItemRef extends AbstractStructRef {
    ...
    constructor (decoder, id, info) {
       ...
       * @type {ID | null}
       */
      this.left = (info & BIT8$1) === BIT8$1 ? readID(decoder) : null;
      /**
       * The item that was originally to the right of this item.
       * @type {ID | null}
       */
      this.right = (info & BIT7$1) === BIT7$1 ? readID(decoder) : null;
      const canCopyParentInfo = (info & (BIT7$1 | BIT8$1)) === 0;
  ...
  DOMParser$1.prototype.matchTag = function matchTag (dom, context) {
  ...
  DOMParser$1.prototype.matchStyle = function matchStyle (prop, value, context) {
  ...
  prototypeAccessors$1$4.tr.get = function () { return new Transaction$1(this) };
  ...
  const equalYTextPText = (ytext, ptexts) => {
    ...
    return delta.length === ptexts.length && delta.every((d, i) => d.insert === /** @type {any} */ (ptexts[i]).text && keys$1(d.attributes || {}).length === ptexts[i].marks.length && ptexts[i].marks.every(mark => equalAttrs$1(d.attributes[mark.type.name] || {}, mark.attrs)))
  };
  ...
  const pmChanges = [
    ...
    (y, gen, p) => { // delete text
      const insertPos = int32(gen, 0, p.state.doc.content.size);
      const overwrite = min$1(int32(gen, 0, p.state.doc.content.size - insertPos), 2);
